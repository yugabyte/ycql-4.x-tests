...
//            session.execute(""
//                    + "INSERT INTO users (email, firstname, lastname, address) "
//                    + "VALUES ('clun@sample.com', 'Cedrick', 'Lunven', '\"abc\"')");
//            LOGGER.info("+ Insert as a String");
            
            // #1.b But everything is a statement
            session.execute(SimpleStatement.newInstance(
                      "INSERT INTO users (email, firstname, lastname, address) "
                    + "VALUES ('clun2@sample.com', 'Cedrick', 'Lunven', '{\"b\" : 1}')"));
...
//                           .builder("INSERT INTO users (email, firstname, lastname, address) VALUES (?,?,?,?)")
//                           .addPositionalValue("clun3@gmail.com")
//                           .addPositionalValue("Cedrick")
//                           .addPositionalValue("Lunven")
//                           .addPositionalValue("3").build());
...
//            PreparedStatement ps1 = session.prepare("INSERT INTO users (email, firstname, lastname, address) "
//                                                  + "VALUES (?,?,?,?)");
//            BoundStatement bs1 = ps1.bind("clun6@gmail.com", "Cedrick", "Lunven", "\"abc\"");
//            session.execute(bs1);
//            LOGGER.info("+ Insert with PrepareStatements");
            
            
            SimpleStatement statement1 = SimpleStatement.builder("INSERT INTO users (email, firstname, lastname, address) "
                    + "VALUES (?,?,?,?)").setConsistencyLevel(DefaultConsistencyLevel.ONE).build();
            PreparedStatement ps2 = session.prepare(statement1);
            BoundStatement bs2 = ps2.bind("clun12@gmail.com", "Cedrick", "Lunven", "\"pqrs\"");
            session.execute(bs2);
//            
//           
